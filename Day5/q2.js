...
for(let i = 0; i < input.length; i++){
  ...
  let check = false;
  if(Math.abs(cX - bX) === Math.abs(cY - bY)) check = true;
  if(cX != bX && cY != bY && !check) continue;
  if(check){
    let dis = Math.abs(cX - bX)
    let leX, leY;
    for(let a = 0; a <= dis; a++){
      if(cX > bX) leX = cX - a; 
      else if(cX < bX) leX = cX + a;
      if(cY > bY) leY = cY - a 
      else if(cY < bY) leY = cY + a
      covered.push({x:leX,y:leY})
    }
  }else{
    if(cX === bX){
      let lowEnd = cY;
